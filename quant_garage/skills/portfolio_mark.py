"""
portfolio-mark as an importable library function.

Marks a CSV of positions to current fair value. Two modes: 'delayed'
(one REST snapshot per symbol via the resolve_price fallback chain)
and 'live' (WebSocket stream, listens N seconds).
...
try:
    ...
    websocket = None  # live mode unavailable; raise a clear error on use
...
def run(csv_path: str, mode: str = "delayed", listen: int = 30) -> dict:
    """Mark a CSV of positions to fair value.

    Args:
        csv_path: positions CSV with columns ticker,shares[,cost_basis,as_of_date].
        mode: 'delayed' (one REST snapshot per name) or 'live' (WebSocket stream).
        listen: live-mode window in seconds. Default 30.
    """
    if mode not in ("delayed", "live"):
        raise ValueError("mode must be 'delayed' or 'live'")
    if mode == "live" and websocket is None:
        raise RuntimeError(
    ...
    if mode == "delayed":
        return run_delayed(positions)
    return run_live(positions, listen)


# CLI removed — see examples/run-portfolio-mark.py
