"""
slippage-cost as an importable library function.

Takes a CSV of executed fills, runs fill-vs-NBBO slippage analysis,
flags crossed spreads, off-NBBO trades, wide spreads, VWAP slippage,
...
def run(csv_path: str, client_: MassiveClient | None = None) -> dict:
    """Analyze fills vs NBBO. Returns exception-report payload.

    Args:
        csv_path: path to fills CSV. See examples/sample-fills.csv.
    """
    global client
    ...
    if not fills:
        raise ValueError("no fills in input")
...
# CLI removed — see examples/run-slippage-cost.py
