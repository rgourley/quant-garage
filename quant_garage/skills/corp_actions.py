"""
corp-actions-reconciler as an importable library function.

Reads a positions CSV, queries Massive splits + dividends, reconciles
against expected state, emits an exception report of breaks.
...
# references/spinoffs-methodology.md. Callers can override via
# spinoffs_path= on run(); default points to a file next to examples/.
_DEFAULT_SPINOFFS_PATH = os.path.join(
    os.path.dirname(__file__), os.pardir, os.pardir, os.pardir,
    "examples", "spinoffs.json"
)
SPINOFFS_OVERRIDES_PATH = _DEFAULT_SPINOFFS_PATH  # back-compat
...
def run(
    csv_path: str,
    spinoffs_path: str | None = None,
    client_: MassiveClient | None = None,
) -> dict:
    """Reconcile a positions CSV against Massive splits + dividends.

    Args:
        csv_path: path to positions CSV. See examples/sample-positions.csv.
        spinoffs_path: optional path to spinoff overrides JSON.
                       Defaults to examples/spinoffs.json if present.
    """
    global client
    if client_ is not None:
        client = client_
    positions = load_positions(csv_path)
    spinoffs = load_spinoffs_overrides(spinoffs_path or SPINOFFS_OVERRIDES_PATH)
