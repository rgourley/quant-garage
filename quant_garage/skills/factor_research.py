"""
factor-research as an importable library function.

Multi-factor IC + decile-spread study on a US equity universe.

    from quant_garage.skills.factor_research import run, render
    payload = run(universe_size=200, years=3.0)

Real runtime for universe-wide work:
  - Flat-files cold: 8-15 minutes for 5y top-500
  - REST grouped cold: 10-20 minutes
...
def _build_factor_universe(target_size):
    ...
    `quant_garage.build_universe`, which builds a point-in-time
...
def _winsorize_series(s, low=0.01, high=0.99):
    ...
    `quant_garage.winsorize`. The lib version takes Sequence[float]
...
def run(
    ...
    interface: str = "auto",
    debug_factor: bool = False,
    client_: MassiveClient | None = None,
) -> dict:
    """Run multi-factor IC + decile-spread study over a US equity universe.

    Args:
        universe_size: top N by current market cap. Default 200.
        years: backtest window length. Default 3.0.
        end_date: end date YYYY-MM-DD. Default today.
        interface: 'auto' | 'flat-files' | 'rest'. Default 'auto'.
        debug_factor: print debug info to stderr.
        client_: reuse an existing MassiveClient.

    Note: this is a HEAVY workflow. Cold runs take 8-20 minutes; warm cache <2min.
    """
    global client
    ...
    args = SimpleNamespace(
        ...
        end_date=end_date,
        interface=interface,
        debug_factor=debug_factor,
    )

    if interface not in ("auto", "flat-files", "rest"):
        raise ValueError(f"interface must be 'auto', 'flat-files', or 'rest'; got {interface!r}")
    ...
    if args.interface in ("auto", "flat-files"):
        ...
        if has_ff:
            ...
            if args.interface == "flat-files":
                raise RuntimeError(
                    "interface='flat-files' requested but key is not entitled."
                )
...
# CLI removed — see examples/run-factor-research.py
