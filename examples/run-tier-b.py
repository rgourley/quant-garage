#!/usr/bin/env python3
"""
Tier B run of the earnings-drilldown skill against a user-supplied ticker.
...
Reads the peer override map from references/peer-reaction.md so the script
and the documented methodology stay in sync.
Writes output to examples/{ticker_lower}-tier-b-output.md (gitignored).
"""
import os
import re
import sys
...
if len(sys.argv) < 2:
    print("Usage: run-tier-b.py TICKER", file=sys.stderr)
    sys.exit(1)
...
SKILL_DIR = os.path.normpath(
    os.path.join(os.path.dirname(__file__), "..", "skills", "earnings-drilldown")
)
PEER_DOC = os.path.join(SKILL_DIR, "references", "peer-reaction.md")


def load_peer_overrides():
    """
    Parse the PEER_OVERRIDES dict from references/peer-reaction.md so the
    script and the documented methodology never drift. We do a line-by-line
    parse of the python-formatted block (no code execution) since the format
    is stable: `"AAPL":  ["NVDA", "MSFT", ...],`.
    """
    with open(PEER_DOC, "r") as f:
        src = f.read()
    block_match = re.search(
        r"PEER_OVERRIDES\s*=\s*\{(.*?)\n\}", src, re.DOTALL
    )
    if not block_match:
        raise RuntimeError(f"Could not find PEER_OVERRIDES block in {PEER_DOC}")
    block = block_match.group(1)
    out = {}
    # Match: "TICKER": ["A", "B", "C"],
    row_pattern = re.compile(
        r'"([A-Z][A-Z0-9.\-]*)"\s*:\s*\[((?:\s*"[A-Z0-9.\-]+"\s*,?\s*)+)\]'
    )
    for m in row_pattern.finditer(block):
        key = m.group(1)
        peers = re.findall(r'"([A-Z0-9.\-]+)"', m.group(2))
        out[key] = peers
    if not out:
        raise RuntimeError(f"Parsed empty PEER_OVERRIDES from {PEER_DOC}")
    return out


PEER_OVERRIDES = load_peer_overrides()
...
print(f"Fetching ticker metadata for {TICKER}...", file=sys.stderr)
...
print(f"Fetching {TICKER} daily aggregates {start_date} to {end_date}...", file=sys.stderr)
...
# NOTE: Per-script use of lastQuote.p (quote-mid) for spot bypasses lastTrade.
# `resolve_price()` from the lib doesn't currently cover lastQuote, so the
# inline read stays. See AUDIT.md N2.
...
if not spot:
    spot = spot_snap.get("prevDay", {}).get("c")
...
projected_session = (
    prints_with_reaction[-1]["session"] if prints_with_reaction else "AMC"
)
...
tier_caveats = [
    ...
    "EPS actuals are GAAP (from 10-Q/10-K); Benzinga's Tier A actuals are non-GAAP adjusted.",
...
header_extras = []
header_extras.append(f"Next print (projected): {payload['print']['date']} {projected_session}")
...
# Take generation, per references/rendering.md
def render_mispricing_phrase(iv_r):
    """Return the 'rich/cheap/fair' take string per rendering.md rules."""
    m = iv_r["mispricing_pct"]
    imp = iv_r["implied_move_pct"]
    real = iv_r["realized_avg_pct"]
    n = iv_r["n_quarters"]
    if m is None:
        return None
    if m > 0.15:
        if abs(m) > 1.0:
            pp = (imp - real) * 100
            return (
                f"Straddle prices {pp:.1f}pp above {n}q realized "
                f"(implied ±{imp * 100:.1f}%, realized ±{real * 100:.1f}%). "
                f"Premium sellers have a setup."
            )
        return (
            f"Straddle is {m * 100:.0f}% rich vs {n}q realized "
            f"(implied ±{imp * 100:.1f}%, realized ±{real * 100:.1f}%). "
            f"Premium sellers have a setup."
        )
    if m < -0.15:
        if abs(m) > 1.0:
            pp = (real - imp) * 100
            return (
                f"Straddle prices {pp:.1f}pp below {n}q realized "
                f"(implied ±{imp * 100:.1f}%, realized ±{real * 100:.1f}%). "
                f"Premium buyers have a setup."
            )
        return (
            f"Straddle is {abs(m) * 100:.0f}% cheap vs {n}q realized "
            f"(implied ±{imp * 100:.1f}%, realized ±{real * 100:.1f}%). "
            f"Premium buyers have a setup."
        )
    return (
        f"Straddle is fair vs {n}q realized "
        f"(implied ±{imp * 100:.1f}%, realized ±{real * 100:.1f}%)."
    )
...
if payload["implied_vs_realized"]:
    phrase = render_mispricing_phrase(payload["implied_vs_realized"])
    if phrase:
        take_parts.append(phrase)
if not take_parts:
...
# PEAD section — render bucket lines, omit any bucket whose data is null
pead_pos = pead.get("on_positive_reactions")
pead_neg = pead.get("on_negative_reactions")
pead_pos_renderable = pead_pos and pead_pos["avg_t5_return_pct"] is not None
pead_neg_renderable = pead_neg and pead_neg["avg_t5_return_pct"] is not None

if pead_pos_renderable or pead_neg_renderable:
    lines.append("Post-earnings drift (T+1 to T+5, SPY-adjusted, bucketed by reaction sign)")
    if pead_pos_renderable:
        b = pead_pos
        if b["n"] < 4:
...
if peer_reaction_block:
    ...
    if pr["avg_peer_return_on_positive_reaction_pct"] is not None:
        lines.append(
            f"- Peers on positive {TICKER} reactions: {fmt_signed_pct(pr['avg_peer_return_on_positive_reaction_pct'])} avg"
        )
    if pr["avg_peer_return_on_negative_reaction_pct"] is not None:
        lines.append(
            f"- Peers on negative {TICKER} reactions: {fmt_signed_pct(pr['avg_peer_return_on_negative_reaction_pct'])} avg"
        )
    if pr["top_peers"]:
...
# 14. Write output
out_name = f"{TICKER.lower()}-tier-b-output.md"
out_path = os.path.join(os.path.dirname(__file__), out_name)
with open(out_path, "w") as f:
    f.write(f"# Tier B run: earnings-drilldown {TICKER}\n\n")
