#!/usr/bin/env python3
"""
Reference implementation of the backtest-data-prep skill.

Builds a clean OHLCV parquet for a US equity universe across an
arbitrary window. Emits three files to the output directory:

  ohlcv.parquet      - one row per (ticker, trading day)
  manifest.md        - human-readable run record
  edge-cases.log     - line-delimited JSON, one entry per anomaly

And prints two outputs to stdout:

  - canonical JSON matching skills/backtest-data-prep/output-schema.json
  - the dataset-mode rendered summary

Usage:
    python3 examples/run-backtest-data-prep.py --universe top100 \\
        --window 2022-06-25..2026-06-25 --out examples/backtest-data/
...
needs distinct credentials: MASSIVE_S3_ACCESS_KEY + MASSIVE_S3_SECRET_KEY
(generate in your Massive dashboard under "Flat Files"). POLYGON_S3_*
also accepted as a fallback for users following legacy Polygon docs.
If neither is set, the script falls back to using MASSIVE_API_KEY as
both S3 halves with a warning — that pattern usually 403s and the
script then falls back to the REST grouped-daily path.
...
def main():
    p = argparse.ArgumentParser(description="backtest-data-prep reference implementation")
    p.add_argument("--universe", type=str, default="top100",
                   help="top100 | top500 | top1000 | sp500 | custom:path/to/tickers.csv")
    p.add_argument("--window", type=str, required=True,
    ...
    if ws > we:
        raise SystemExit("--window start must be <= end")
