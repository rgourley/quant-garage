#!/usr/bin/env python3
"""
Reference implementation of the hedge-suggester skill.

risk-report flags that a single position carries most of the book's
variance. options-flow shows what other traders are doing. Neither one
proposes what to actually do about a concentrated long. hedge-suggester
closes that loop: it takes one long position and proposes concrete,
live-priced option overlays, ranked by how cheaply each one buys downside
protection.

The workflow:

  1. Resolve the underlying's current price (snapshot, best-price chain).
  2. Pull the options chain around the horizon expiry (nearest listed
     expiry at or beyond --horizon-days) with bid/ask, open interest,
     implied vol, and greeks per contract.
  3. Construct five standard overlays against the LONG position and price
     each from chain mids:
       - Covered call    (sell ~1 OTM call per 100 shares)
       - Protective put  (buy ~1 ATM/slightly-OTM put per 100 shares)
       - Collar          (protective put financed by a covered call)
       - Put spread      (buy near-ATM put, sell a lower put)
       - Ratio put spread (buy 1 put, sell 2 further-OTM puts) -- tail risk
  4. Rank by cost per dollar of downside protected. Cheaper insurance
     ranks higher; the tradeoff is labelled.
  5. Caveat every structure (liquidity floors, tail risk) and the run as a
     whole (earnings proximity not fetched, IV-vs-realized context).

Two output layers from one run:
  Layer 1: canonical JSON matching skills/hedge-suggester/output-schema.json
  Layer 2: rendered table of structures + the take line

Usage:
    python3 examples/run-hedge-suggester.py --ticker ALLO --shares 1000
    python3 examples/run-hedge-suggester.py --ticker AAPL --notional 250000 \
        --risk-tolerance low --horizon-days 60 --format both
    python3 examples/run-hedge-suggester.py --ticker ALLO --shares 1000 --sleep 13

Reads MASSIVE_API_KEY from env. Needs Stocks Starter (underlying snapshot
and aggregates) plus the Options Developer add-on (options chain, greeks,
OI, bid/ask), exactly like options-flow. On a rate-limited key pass
--sleep 13 so the aggregate pull stays under the 5-calls/min cap.

This proposes, it does not advise. Mid-price fills are optimistic, greeks
are point-in-time, and assignment / early-exercise risk is real. See
skills/hedge-suggester/references/methodology.md.
"""
...
# ----- Tunables (documented in references/methodology.md) -----

CONTRACT_MULTIPLIER = 100          # shares per option contract
OI_FLOOR = 100                     # skip/flag strikes with OI below this
SPREAD_PCT_CEILING = 0.15          # flag strikes with (ask-bid)/mid above this
OTM_CALL_TARGET = 0.05             # covered-call strike ~5% OTM
PROTECTIVE_PUT_TARGET = 0.00       # protective-put strike ~ATM (at/below spot)
SPREAD_LOW_TARGET = 0.10           # put-spread short leg ~10% below spot
RATIO_LOW_TARGET = 0.15            # ratio short legs ~15% below spot
STRIKE_BAND_PCT = 0.30             # pull strikes within +/-30% of spot
REALIZED_VOL_WINDOW = 20           # trading days for realized-vol proxy
IV_EXPENSIVE_RATIO = 1.30          # ATM IV / realized vol above this = expensive
IV_CHEAP_RATIO = 1.00             # below this = cheap
...
def fetch_daily_aggs(ticker: str, calendar_days: int) -> list[dict]:
    """Daily {date, close} records for `ticker`, ascending.

    Reuses run-macro-basket.py's rate-limit-aware pattern: rate limits are
    FetchError subclasses that otherwise look like "no data", so detect
    them, cool down once, retry, and flag the ticker so the caller caveats
    loudly rather than silently degrading the IV context.
    """
    end = TODAY
...
def get_spot(ticker: str) -> tuple[float | None, str]:
    """Current underlying price via lib.resolve_price. Returns (price, source)."""
    try:
...
def get_chain(ticker: str, spot: float, horizon_days: int) -> list[dict]:
    """Options chain around the horizon expiry within +/-STRIKE_BAND_PCT of spot.

    Mirrors options-flow's get_chain: same /v3/snapshot/options/{ticker}
    endpoint, same strike/expiry query params, so the same Options Developer
    entitlement covers it. Pulls a window of expiries [horizon-14, horizon+45]
    days so the nearest listed expiry at or beyond the horizon is present even
    when the exact horizon day is not a listed expiry. Single page (limit 250)
    is enough for one name's near-dated chain in a +/-30% strike band; add
    client.paginate for very wide bands.
    """
    strike_lo = int(spot * (1 - STRIKE_BAND_PCT))
    strike_hi = int(spot * (1 + STRIKE_BAND_PCT) + 1)
    exp_from = (TODAY + timedelta(days=max(0, horizon_days - 14))).isoformat()
    exp_to = (TODAY + timedelta(days=horizon_days + 45)).isoformat()
    path = (
...
def parse_contract(o: dict) -> dict | None:
    """Normalize one v3 options-snapshot contract into the fields we price on.

    v3 snapshot shape (see options-flow SKILL.md):
      details:      {ticker, contract_type, strike_price, expiration_date}
      last_quote:   {bid, ask, midpoint}
      day:          {volume, vwap, close}
    ...
    if mid is None and bid is not None and ask is not None and ask >= bid:
        mid = (bid + ask) / 2.0
    # Fall back to the day close only for a mid when there is no quote.
    if mid is None:
        day = o.get("day") or {}
...
def choose_expiry(contracts: list[dict], horizon_days: int) -> str | None:
    """Nearest listed expiry at or beyond horizon_days; else the latest listed."""
...
def nearest_strike(strikes: list[float], target: float,
                   prefer: str = "any") -> float | None:
    """Nearest available strike to `target`.

    prefer='below' restricts to strikes <= target (else nearest overall),
    prefer='above' restricts to strikes >= target (else nearest overall).
    """
    if not strikes:
        return None
    pool = strikes
...
# ----- Payoff math (see references/methodology.md) -----

def _leg_expiry_value(leg: dict, s: float) -> float:
    """Intrinsic value at expiry for one leg, in dollars (signed, x100 x qty)."""
    k = leg["strike"]
    if leg["type"] == "call":
...
def structure_pnl(legs: list[dict], shares: int, s0: float,
                  net_cost_usd: float, s: float) -> float:
    """Combined position (long stock + option legs) P&L at price `s` at expiry."""
    stock_pnl = (s - s0) * shares
    opt = sum(_leg_expiry_value(leg, s) for leg in legs)
    return stock_pnl + opt - net_cost_usd


def leg_premium_usd(leg: dict) -> float | None:
    """Signed premium in dollars for one leg (buy = debit +, sell = credit -)."""
    if leg["mid"] is None:
        return None
    sign = 1.0 if leg["action"] == "buy" else -1.0
...
def leg_greek_contrib(leg: dict, greek: str) -> float | None:
    """Signed position greek contribution for one leg (per-share greek x100 x qty)."""
    g = leg.get(greek)
    if g is None:
        return None
    sign = 1.0 if leg["action"] == "buy" else -1.0
...
def liquidity_flags(legs: list[dict]) -> list[str]:
    """Per-leg liquidity checks: OI floor and bid-ask spread ceiling."""
    flags: list[str] = []
    for leg in legs:
...
def build_structure(name: str, legs: list[dict], shares: int, s0: float,
    ...
    """Price one structure end to end. Returns None if any leg lacks a mid."""
    premiums = [leg_premium_usd(leg) for leg in legs]
    if any(p is None for p in premiums):
        return None
    net_cost_usd = float(sum(premiums))            # debit +, credit -
    net_cost_per_share = net_cost_usd / shares if shares else 0.0
    notional = s0 * shares
    net_cost_pct = (net_cost_usd / notional) if notional else None

    # Combined-position payoff over a 0 .. 3*S0 grid.
    grid = [s0 * i / 200.0 for i in range(0, 601)]  # 0 .. 3*S0, step S0/200
    pnls = [structure_pnl(legs, shares, s0, net_cost_usd, s) for s in grid]
    max_loss_usd = min(pnls)
    max_gain_usd = max(pnls)
    # Uncapped upside: P&L is still STRICTLY climbing at the top of the grid.
    # A short call flattens P&L above its strike (difference ~ 0), which is a
    # cap, not uncapped. Threshold well below one step of naked-stock slope
    # (shares * step) but well above the flat-top zero.
    step = s0 / 200.0
    slope_top = pnls[-1] - pnls[-2]
    uncapped_upside = slope_top > shares * step * 0.25

    # Conventional breakeven: credit structures break even on the downside
    # (spot minus premium collected); debit structures on the upside (spot
    # plus premium paid).
    if net_cost_usd < 0:
        breakeven = s0 + net_cost_per_share          # < s0 (credit)
    else:
        breakeven = s0 + net_cost_per_share          # > s0 (debit)

    # Downside protected: the insured region where the structure pays off on
    # the way down, in dollars. See methodology.md for the interpretation.
    ...
    # Position greeks: stock delta = shares; options scaled per leg.
    deltas = [leg_greek_contrib(leg, "delta") for leg in legs]
    gammas = [leg_greek_contrib(leg, "gamma") for leg in legs]
    thetas = [leg_greek_contrib(leg, "theta") for leg in legs]
...
def find(contracts: list[dict], ctype: str, strike: float, expiry: str) -> dict | None:
    for c in contracts:
...
def leg(contract: dict, action: str, qty: int) -> dict:
    d = dict(contract)
    d["action"] = action
    d["qty"] = qty
    return d
...
# ----- IV / realized-vol context -----

def realized_vol(rows: list[dict], window: int) -> float | None:
    """Annualized close-to-close realized vol over the last `window` bars."""
...
def iv_context(atm_iv: float | None, rv: float | None) -> dict:
    """Is protection expensive right now, proxied by ATM IV vs realized vol."""
...
def build_payload(args: argparse.Namespace) -> dict:
    ...
    # 1. Spot.
    spot, spot_source = get_spot(ticker)
    sources.append({
    ...
    if spot is None or spot <= 0:
        return {
            ...
            "structures": [],
            "ranking": [],
            "iv_context": iv_context(None, None),
    ...
    # Position sizing: shares, or shares implied by notional.
    if args.shares is not None:
        shares = int(args.shares)
    else:
        shares = int(args.notional // spot)
    if shares <= 0:
        raise SystemExit("resolved share count is zero; check --shares/--notional")
    ...
    # 2. Chain around the horizon expiry.
    raw = get_chain(ticker, spot, horizon_days)
    sources.append({
    ...
    contracts = [c for c in (parse_contract(o) for o in raw) if c is not None]

    # 3. IV context from underlying aggregates (rate-limit-aware pull).
    agg_rows = fetch_daily_aggs(ticker, calendar_days=int(REALIZED_VOL_WINDOW * 2.5) + 30)
    sources.append({
        "endpoint": f"/v2/aggs/ticker/{ticker}/range/1/day/{{from}}/{{to}}",
        "fetched_at": utcnow_iso(),
        "context": "underlying daily closes for realized-vol / IV context",
    })
    rv = realized_vol(agg_rows, REALIZED_VOL_WINDOW)
    if _RATE_LIMITED:
        tier_caveats.append(
    ...
    expiry = choose_expiry(contracts, horizon_days)
    if expiry is None:
        tier_caveats.append(
        ...
        return {
            ...
            "position": {
                ...
                "sized_from": "shares" if args.shares is not None else "notional",
            },
    ...
    # ATM implied vol for the IV context: nearest put/call to spot with an IV.
    atm_iv = None
    ...
    for c in atm_candidates:
        if c.get("iv") is not None:
            atm_iv = c["iv"]
            break
    ivc = iv_context(atm_iv, rv)

    # Strike selection.
    kc = nearest_strike(call_strikes, spot * (1 + OTM_CALL_TARGET), prefer="above")
    kp = nearest_strike(put_strikes, spot * (1 - PROTECTIVE_PUT_TARGET), prefer="below")
    kp_low_spread = nearest_strike(put_strikes, spot * (1 - SPREAD_LOW_TARGET), prefer="below")
    kp_low_ratio = nearest_strike(put_strikes, spot * (1 - RATIO_LOW_TARGET), prefer="below")
    ...
    # Covered call: sell 1 OTM call per 100 shares.
    if kc is not None:
        c_call = find(exp_contracts, "call", kc, expiry)
        if c_call and c_call.get("mid") is not None:
            legs = [leg(c_call, "sell", n_contracts)]
    ...
    # Protective put: buy 1 ATM/slightly-OTM put per 100 shares.
    if kp is not None:
        c_put = find(exp_contracts, "put", kp, expiry)
        if c_put and c_put.get("mid") is not None:
            legs = [leg(c_put, "buy", n_contracts)]
            st = build_structure(
    ...
    if kp is not None and kp_low_spread is not None and kp_low_spread < kp:
        c_hi = find(exp_contracts, "put", kp, expiry)
        c_lo = find(exp_contracts, "put", kp_low_spread, expiry)
        if c_hi and c_lo and c_hi.get("mid") is not None and c_lo.get("mid") is not None:
            legs = [leg(c_hi, "buy", n_contracts), leg(c_lo, "sell", n_contracts)]
            st = build_structure(
    ...
    # Ratio put spread: buy 1 put, sell 2 further-OTM puts. Tail risk flagged.
    if kp is not None and kp_low_ratio is not None and kp_low_ratio < kp:
        c_hi = find(exp_contracts, "put", kp, expiry)
        c_lo = find(exp_contracts, "put", kp_low_ratio, expiry)
        if c_hi and c_lo and c_hi.get("mid") is not None and c_lo.get("mid") is not None:
            legs = [leg(c_hi, "buy", n_contracts), leg(c_lo, "sell", 2 * n_contracts)]
            st = build_structure(
    ...
    # 4. Rank by cost per dollar protected (cheaper protection ranks higher).
    ...
    take = compose_take(risk_tolerance, horizon_days, structures, ranking, spot, ivc)
...
def compose_take(risk_tolerance: str, horizon_days: int, structures: list[dict],
                 ranking: list[dict], spot: float, ivc: dict) -> str:
    """Recommend the structure that best fits the stated risk tolerance."""
    if not structures:
        return "No structures could be priced from the chain; no hedge to recommend."

    # Risk-tolerance -> preferred structure order.
    prefs = {
    ...
    # Add the cheapest-per-protection note and a collar/upside contrast.
    extras = []
...
def render(payload: dict) -> str:
    ...
    # Per-structure legs, greeks, and caveats.
    lines.append("")
    for s in ordered:
...
def main() -> None:
    ap = argparse.ArgumentParser(description="hedge-suggester reference")
    ...
    ap.add_argument("--sleep", type=float, default=0.0,
                    help="Seconds between aggregate calls. Rate-limited keys cap "
                         "at 5 calls/min; use --sleep 13.")
