#!/usr/bin/env python3
"""
CLI wrapper for the guidance-tracker skill.

    from quant_garage.skills.guidance_tracker import run, render
    payload = run("NVDA", lookback_days=540)

Reads MASSIVE_API_KEY from env. Requires Benzinga Corporate Guidance add-on.
"""
...
def main() -> int:
    ap = argparse.ArgumentParser(
        prog="run-guidance-tracker",
        description="Track corporate guidance history (raises/lowers/reaffirms) for a ticker via Benzinga Corporate Guidance.",
    )
