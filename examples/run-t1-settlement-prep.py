#!/usr/bin/env python3
"""
Reference implementation of the t+1-settlement-prep skill.

Takes a CSV of trades that haven't settled yet, computes the T+1
business-day settlement date for each, walks Massive's holiday and
corporate-action calendars to flag settlement risk, and emits a
dual-layer output:
...
Writes the run to examples/t1-settlement-prep-output.md (gitignored).
"""
import csv
import json
import os
...
def main():
    if len(sys.argv) < 2:
        print("Usage: run-t1-settlement-prep.py <trades.csv>", file=sys.stderr)
        sys.exit(1)
    csv_path = sys.argv[1]
    trades = load_trades(csv_path)
    if not trades:
        print("ERROR: no trades in input", file=sys.stderr)
        sys.exit(1)
